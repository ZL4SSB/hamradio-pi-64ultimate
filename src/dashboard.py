...
class HamRadioPiUltimate:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.version_info = ConfigService.load_json(
            VERSION_FILE,
            {"version": "0.3.4", "edition": "Community"},
        )
    ...
    def build_interface(self) -> None:
        ...
        ttk.Label(main, text="Build your station, not your software list.", font=("Arial", 11)).pack(pady=(3, 15))
        ...
        ttk.Label(
            main,
            text=f"HamRadio-Pi Ultimate {edition} Edition — Version {version}",
            font=("Arial", 9),
        ).pack(pady=(12, 0))

    def panel(self, parent, title, row, column):
        panel = ttk.LabelFrame(parent, text=title, padding=15)
        panel.grid(row=row, column=column, padx=8, pady=8, sticky="nsew")
        return panel
    ...
    def build_station_panel(self, parent) -> None:
        ...
        self.add_row(panel, 1, "Grid", self.grid_value)
        self.add_row(panel, 2, "Station Type", self.station_type_value)
        self.add_row(panel, 3, "Current UTC", self.utc_time_value)
        self.add_row(panel, 4, "Local Time", self.local_time_value)
        ttk.Button(panel, text="Edit Station Profile", command=self.open_station_wizard).grid(
    ...
    def build_status_panel(self, parent) -> None:
        ...
        ttk.Label(panel, text="Station Health", font=("Arial", 11, "bold")).grid(row=0, column=0, sticky="w", pady=6)
        ttk.Label(panel, text="Not checked").grid(row=0, column=1, sticky="e", pady=6)
        self.add_row(panel, 1, "Connected Radio", self.radio_value)
        self.add_row(panel, 2, "Connected SDR", self.sdr_value)
        apps = self.catalogue.get("applications", [])
        count = len(apps) if isinstance(apps, list) else 0
        ttk.Label(panel, text="Catalogue Applications", font=("Arial", 11, "bold")).grid(row=3, column=0, sticky="w", pady=6)
        ttk.Label(panel, text=str(count)).grid(row=3, column=1, sticky="e", pady=6)
        panel.columnconfigure(1, weight=1)

    def build_information_panel(self, parent) -> None:
        panel = self.panel(parent, "Radio Information", 1, 0)
        for row, item in enumerate(["Latest DX Cluster Spots", "Solar Conditions", "Propagation", "Weather"]):
            ttk.Label(panel, text=item, font=("Arial", 11, "bold")).grid(row=row, column=0, sticky="w", pady=8)
            ttk.Label(panel, text="Coming soon").grid(row=row, column=1, sticky="e", pady=8)
        panel.columnconfigure(1, weight=1)
    ...
    def build_tools_panel(self, parent) -> None:
        ...
        for index, (text, command) in enumerate(buttons):
            ttk.Button(panel, text=text, command=command).grid(
                row=index // 2, column=index % 2, sticky="ew", padx=6, pady=8
            )
    ...
    def add_row(parent, row, title, value) -> None:
        ttk.Label(parent, text=title, font=("Arial", 11, "bold")).grid(row=row, column=0, sticky="w", pady=6)
        ttk.Label(parent, textvariable=value).grid(row=row, column=1, sticky="e", pady=6)
    ...
    def refresh_station_information(self) -> None:
        ...
        self.station_type_value.set(self.station.get("station_type") or "Not selected")
        self.radio_value.set(self.station.get("preferred_radio") or "Not configured")
        self.sdr_value.set(self.station.get("preferred_sdr") or "Not configured")

    def update_clocks(self) -> None:
        self.utc_time_value.set(datetime.now(timezone.utc).strftime("%H:%M:%S UTC"))
        self.local_time_value.set(datetime.now().astimezone().strftime("%H:%M:%S"))
    ...
    def open_station_wizard(self) -> None:
        self.run_python_tool(STATION_WIZARD, "Station Setup Wizard")
        self.refresh_station_information()

    def show_station_builder(self) -> None:
        self.run_python_tool(STATION_BUILDER, "Station Builder")
        self.refresh_station_information()

    def open_updater(self) -> None:
        self.run_python_tool(UPDATER, "Update Manager")

    def open_settings(self) -> None:
        self.run_python_tool(SETTINGS, "Settings")
        self.refresh_station_information()

    def open_wpsd_builder(self) -> None:
        if platform.system() != "Linux":
            messagebox.showinfo("WPSD SD Card Builder", "The WPSD SD Card Builder runs on Raspberry Pi OS.")
            return
        if not WPSD_SCRIPT.exists():
            messagebox.showerror("WPSD SD Card Builder", "The WPSD builder script could not be found.")
            return
    ...
    def show_applications(self) -> None:
        ...
        if not isinstance(apps, list):
            apps = []
        names = "\n".join(f"• {item.get('name', 'Unknown')}" for item in apps if isinstance(item, dict))
        messagebox.showinfo("Applications", names or "No applications are currently listed.")

    @staticmethod
    def show_hardware() -> None:
        messagebox.showinfo("Hardware", "Automatic hardware detection will be added soon.")
