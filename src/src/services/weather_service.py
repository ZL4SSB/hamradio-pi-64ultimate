"""Weather and Maidenhead-grid services for HamRadio-Pi Ultimate."""
...
class WeatherError(RuntimeError):
    """Raised when weather settings or requests are invalid."""
...
def resolve_coordinates(
    ...
    """Choose exact coordinates or derive them from the station grid."""

    if not use_station_grid:
        try:
    ...
    if not station_grid.strip():
        raise WeatherError(
            "Enter a station grid square before using grid-based weather."
        )
...
def test_open_meteo(coordinates: Coordinates) -> str:
    """Fetch a small current-weather sample from Open-Meteo."""
    ...
    if not isinstance(current, dict):
        raise WeatherError(
            "Open-Meteo did not return current weather information."
        )
...
def test_openweather(
    ...
    """Test the OpenWeather current-weather endpoint."""
    ...
    if not isinstance(main, dict):
        raise WeatherError(
            "OpenWeather did not return current weather information."
        )
